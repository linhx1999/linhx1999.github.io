...
- 正文标题从 `##` (h2) 开始使用，不要使用 `#`
- 文章页会根据标题自动生成浮动目录，新文章不要再手写 `## TOC`
- 如果是在维护旧文章，保留已有 `## TOC` 即可，页面层会兼容处理，但不建议继续作为新文章模板
...
在博客中使用代码块绘制 ASCII 图表（如流程图、时序图、架构图等）时：

**必须使用英文文字**，避免中英文混排导致的对齐问题。
...
**原因**：中文字符通常是英文字符宽度的 2 倍，在等宽字体环境下会导致表格线错位。
