...
网络基础类综述也可参考 `src/data/blog/2026/tcp-core-concepts.md`：优先把
`TCP`、`UDP`、`ICMP`、握手、窗口、拥塞控制和攻击面放到同一条链路中解释，
避免把缩写释义拆成彼此割裂的术语卡片。
AI Agent / LLM 工程类综述可参考 `src/data/blog/2026/what-is-agent-harness.md`：
先区分模型本身与 harness、workflow、runtime 这类外围系统，再说明长期任务中的失败模式与编排对策，避免把 prompt 技巧、工具接入和多 Agent 分工混成没有边界的经验列表。

涉及协议、标准或外部规范的文章时，优先在正文或参考资料中标明所依据的规范版本或日期；例如
MCP、HTTP、框架 API 这类会演进的主题，不要默认旧行为仍然代表当前标准。
术语密集的文章中，首次出现的英文缩写建议补充全称，例如 `SSE` 对应
`Server-Sent Events`，避免读者只记住缩写而不清楚其原始含义。
如果文章核心在于“标准协议 vs 普通接口”的差异，优先补一个最小普通请求示例作为对照，
再展示协议化后的请求结构，避免只讲抽象概念。
Go 代码块统一使用 Shiki 支持的语言标记 `go`，不要写成 `golang`，以免高亮回退为纯文本。
文章页已提供自动生成的浮动目录，新文章默认不需要再手写 `## TOC`。
旧文章若保留 `## TOC`，页面会兼容并优先显示浮动目录；不建议继续作为新文章写法。
涉及安全协议的文章时，除正常握手或传输流程外，也建议补一句异常场景的影响范围，例如
会话密钥泄露、证书错误或认证失败后会导致什么后果。
